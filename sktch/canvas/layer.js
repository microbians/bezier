...
class Layer extends Canvas {
	...
	render(layer) {
		...
		if ( cells.length<layer.grid.cells.length*.95 ) {
            ...
            if (layer.debug) {
				layer.debug.globalCompositeOperation 	= "source-over"; 	//"source-over";
				layer.debug.clear()
				for (let j=0; j<layer.grid.cells.length; j++) {
					layer.debug.strokeStyle = "rgba(0,0,0,.2)"
					layer.debug.lineWidth=1;
					layer.debug.strokeRect( layer.grid.getX(j), layer.grid.getY(j), layer.grid.cellW, layer.grid.cellH );
				}
			}

			layer.mskCanvas.globalCompositeOperation = "source-over"; 	//"source-over";
			...
			for (let i=0; i<cells.length; i++) {
				...
				let x1 = layer.grid.getX(c);
				let y1 = layer.grid.getY(c);
				layer.mskCanvas.fillRect( x1, y1, layer.grid.cellW, layer.grid.cellH );
				if (layer.debug) {
					layer.debug.fillStyle = "#00A0A0";
					layer.debug.fillRect( x1, y1, layer.grid.cellW, layer.grid.cellH );
				}
			}

            layer.offCanvas.globalCompositeOperation = "source-over"; 	//"source-over";
            for(let e of elms) {
				e.render(layer.offCanvas, 1);
			}

			layer.globalCompositeOperation = "destination-out"; //"source-over";
			layer.copy(layer.mskCanvas)

			layer.offCanvas.globalCompositeOperation = "destination-in"; //"source-over";
