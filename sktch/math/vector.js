...
class Vector {
	...
	per(s) { // PERPENDICULAR ROTATION OF VECTOR A
		if (!s) return new Vector( this.y, -this.x );
	...
	int(B, from, to, cur) { // Interpolate from this to B when curVAL goes fromVAL => toVAL
		...
		return this.mul(1-t).add(B.mul(t));
	}
}
...
Vector.clockwise=function(p1,pc,p2){
	return Vector.isLeft(p1,pc,p2)>0;
}
